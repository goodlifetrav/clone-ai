...
- SHOPIFY SECTION TAGGING — add a data-igualai-section attribute to every top-level section wrapper div/section. Valid values: "announcement-bar" (thin promo bar above nav), "hero" (full hero with big heading + CTA), "product-grid" (show products inside one collection), "collection-list" (clickable tiles each linking to a different collection page), "features" (icon feature grid), "testimonials" (quote/review cards), "lifestyle" (photo editorial strip), "newsletter" (email signup form), "content" (anything else). Example: <section data-igualai-section="hero" class="...">
